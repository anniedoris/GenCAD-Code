...
def convert_h5_to_cadquery(vecs, save_python_dir, save_step_path, use_fixed_decimal, truncate):
    ...
    def cadquery_workplane(sketch_plane_obj, sketch_num):
       ...
       # if any (coord == 1.0 for coord in sketch_plane_obj.x_axis) and any(coord == 1.0 for coord in sketch_plane_obj.normal):
        ...
        if sketch_plane_obj.x_axis[0] == 1.0 and sketch_plane_obj.normal[1] == -1.0:
            plane = 'XZ'
            python_command = f"wp_sketch{sketch_num} = cq.Workplane(\"{plane}\").transformed(offset=({sketch_plane_obj.origin[0]}, {sketch_plane_obj.origin[1]}, {sketch_plane_obj.origin[2]}))\n"    
        else:
    ...
    def cadquery_circle(x, y, r, loop_list, unquantize, extrude_scale):
        if unquantize:
            ...
            r = r*scale
        if len(loop_list) == 0:
            return f".moveTo({x:.{truncate}f}, {y:.{truncate}f}).circle({r:.{truncate}f})" if use_fixed_decimal else f".moveTo({x}, {y}).circle({r})"
        else:
